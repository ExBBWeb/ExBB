...
$smile_url = ROOT_URL.'/media/images/smiles/';
$smile_dir = ROOT.'/media/images/smiles/';

// Формируем набор смайликов
$pak = file($smile_dir . 'Set_Smiles_YarNET.pak');
$smiles = array();
foreach ($pak as $val) {
    $val = trim($val);
    if (! $val || '#' === $val{0}) { continue; }
    list($gif, $alt, $symbol) = explode('=+:',$val);
    $smiles[$symbol] = '<img src="' . $smile_url . htmlspecialchars($gif) . '" alt="'
